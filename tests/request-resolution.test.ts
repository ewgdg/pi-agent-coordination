...
test("Answer presentation metadata is typed and cannot decorate unrelated Message receipts", () => {
	for (const resumedRequestMessageId of [false, 42, "", {}, undefined]) {
		...
		const toolCallId = "invalid-presentation";
		const entryId = manager.appendMessage(fauxAssistantMessage(fauxToolCall(
			"agent_message", input, { id: toolCallId },
		), { stopReason: "toolUse" }));
		const source = { agentId, entryId, toolCallId };
		manager.appendMessage({
			role: "toolResult", toolName: "agent_message", toolCallId,
			content: [{ type: "text", text: "Done." }], isError: false, timestamp: Date.now(),
			details: { messageId: deriveMessageIdentity(source), requestMessageId: requestId,
				messageStatus: "sent", resumedRequestMessageId },
		});
		assert.throws(() => inspectAgentMessageAuthorResult({
	...
	const agentId = manager.getSessionId();
	manager.appendCustomEntry(AGENT_IDENTITY_CUSTOM_TYPE, { agentId });
	const input = { operation: "send" as const, targetAgent: "target", content: "Direction." };
	const toolCallId = "unrelated-presentation";
	const entryId = manager.appendMessage(fauxAssistantMessage(fauxToolCall(
		"agent_message", input, { id: toolCallId },
	), { stopReason: "toolUse" }));
	const source = { agentId, entryId, toolCallId };
	manager.appendMessage({
		role: "toolResult", toolName: "agent_message", toolCallId,
		content: [{ type: "text", text: "Sent." }], isError: false, timestamp: Date.now(),
		details: { messageId: deriveMessageIdentity(source), targetAgentId: "target",
			messageStatus: "sent", resumedRequestMessageId: null },
	});
	assert.throws(() => inspectAgentMessageAuthorResult({
		authorAgentId: agentId, transcript: transcriptFromSessionManager(manager).inspect(),
		source, input, resolvedTargetAgentId: "target",
	}), /author result has an invalid shape/);
});
