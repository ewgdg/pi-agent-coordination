...
A bounded runtime response to an exactly recognized model-generation fault, available only where the provider or Agent Session adapter guarantees safe continuation in the same exact Agent Run without repeating a committed tool effect. It never reconciles a tool call, reconstructs work in a successor Run, or retries an ambiguous generation outcome. Unresolved or indeterminate reconciliation starts Moderator handling only when an Answer Obligation remains.
