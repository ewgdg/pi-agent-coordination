...
The optional caller-authored `config` object in a context-isolated canonical Agent Spawn tool call. It remains in that canonical parent tool call and, on every fresh Runtime, overlays the child's captured creation rules. It may override model, thinking, working directory, the ordinary-tool allowlist, skills, extension inheritance, and Project Context. The allowlist is a capability ceiling; Pi and extensions own the active subset and its order. It is never expanded into a durable effective configuration. A Conversation Fork rejects Agent Spawn Configuration.
...
**Agent Request**:
A Request targeting a known Agent in the same Workflow. Requests become eligible in admission order among candidates allowed by foreground ancestry. A descendant may suspend a cooperatively waiting foreground; siblings and unrelated Requests remain queued. Each Request keeps its authored Delivery mode. Its Request Message identity, named `requestMessageId`, is the public identity for Cancellation, retry, and retrieval.
...
**Answer Obligation**:
One responder duty created by Agent Request Delivery and ended by committing its correlated Agent Answer or receiving Request Cancellation. The latest unresolved frame is foreground; enclosing frames remain suspended. Runtime derives Request ancestry and dependency ownership from the author's source-time foreground. While the obligation is active, an ordinary Message to that Request's requester is rejected without creating a Message; the responder instead keeps provisional findings local, commits the curated Agent Answer, or authors a reverse Agent Request when requester input is needed. Automatic moderation protects only work with an unresolved Answer Obligation.

**Agent Answer**:
An immutable responder-authored Message explicitly targeting the foreground Request by Message ID or unique incoming suffix. Its route follows from that Request; commitment resolves only that frame and ends execution before any resumed frame continues.
...
**Agent Wait**:
One explicit parameterless join over the caller's foreground-owned outstanding Agent Requests (or independent work without a foreground). When its committed sequential tool call begins execution, it fixes a non-empty snapshot in canonical Request authoring order, including unanswered Requests and committed Answers without requester-side Delivery proof while excluding cancellations, prior Answer Deliveries, and later Request sources. It renews same-identity delivery scheduling for captured unanswered, undelivered Requests through shared Message admission, which may start a Dormant recipient. Existing queues and reservations coalesce; causal ordering and Holds remain in force. Already delivered Requests are awaited without replay, and committed Answers remain retrievable regardless of responder phase. It parks the exact Run until every Request in that snapshot has a canonical Agent Answer, primary interactive human input redirects that Agent, or an eligible inbound Agent Request needs that Run's attention; a child caller releases its child execution slot while parked, while Owner and Moderator Runs never consume child capacity. At that boundary a fully committed aggregate wins; otherwise the Wait completes as Preempted without consuming Answers. A committed aggregate becomes requester-side Delivery proof for Answers not already delivered, while Preemption creates no Answer Delivery proof and a later Wait takes a fresh outstanding snapshot. Ordinary Messages neither satisfy Agent Requests nor preempt Agent Wait, and explicit human follow-up input remains queued. Live notification, the pending Promise, and five-second transcript reconciliation are volatile machinery. Scheduling reconciliation during a parked Wait stays within the recipient Runs captured or started by that call and cannot revive terminated or failed Runs or transfer backlog to successors. Evidence or admission failure reports the affected Request instead of silently stranding it. A successor caller Run may issue a fresh Agent Wait to renew intent or retrieve still-outstanding committed Answers; unfinished Wait calls are not reconstructed, and cold bootstrap remains passive.
_Avoid_: Answer cache, Request retry, durable Wait

**Agent Wait Preemption**:
The non-error completion of one Agent Wait when primary interactive human input redirects the waiting Agent or an eligible inbound Agent Request requires its parked exact Run. Human input first enters Pi's native steering queue, then preemption resumes the Wait so its result cannot start another model turn ahead of that input; an inbound Request instead reserves and commits its Delivery before the next model generation. Preemption reacquires child execution capacity when the caller is a child and leaves every Answer available for ordinary Delivery or a fresh later Wait. An inbound Request must descend from the current foreground when one exists; a delivered Request Cancellation may also preempt; ordinary Agent Messages and explicit human follow-up input remain queued.
_Avoid_: interruption, failure, Answer retrieval

**Request Cancellation**:
An immutable requester-authored Message withdrawing one exact Agent Request, named at the authoring boundary by that Request's Message identity. It ends the requester's wait when committed and the responder's Answer obligation when delivered, without retracting facts or stopping work. An undelivered Request is suppressed without waking its responder; cancellation of a delivered Request may start a dormant responder through ordinary Message delivery. Cancellation removes only its named frame, preserving nested frames. Unfinished outgoing dependencies pass to the nearest surviving original enclosing frame or independent work. Cancellation remains one hop and never grants authority over the responder's Requests.
