...
**Dependency Deadlock**:
A live closed component of settled Agent Runs whose foreground dependencies remain within the component, with no actionable input or external progress source. Ineligible queued Delivery and upstream dependants are not progress. It is a transient observation that clears when its predicate changes, grants no additional authority, and is not reconstructed after host loss.
