...
**Answer Retrieval**:
Requester-initiated delivery of an already-committed Agent Answer through the model-visible result of retrying its Request or waiting for selected Request Answers. The result is the Answer's recipient-side delivery proof and transports the responder's immutable Answer without impersonating the responder or authoring another Message. Retrieval may win while direct Answer Delivery remains only scheduler-held; once direct Delivery owns a frozen or dispatched scheduling reservation, retrieval waits for its proof rather than competing with it. Native tool-result commitment re-arbitrates a previously selected retrieval against current direct Delivery reservation and proof.

**Agent Wait**:
One explicit join over a non-empty ordered selection of Agent Requests authored by the caller. Its sequential tool call parks the exact Run and releases Workflow execution capacity until every selected Request has a canonical Agent Answer or an eligible inbound Agent Request needs that Run's attention. At that boundary a fully committed selected aggregate wins; otherwise the inbound Request is delivered and the Wait completes as Preempted without consuming selected Answers. A committed aggregate becomes requester-side Delivery proof for Answers not already delivered, while Preemption creates no Answer Delivery proof and permits a later Wait over the same Request identities. Ordinary Messages do not preempt Agent Wait. Live notification, the pending Promise, and five-second transcript reconciliation are volatile machinery. A successor Run may issue a new Agent Wait to retrieve committed Answers, but unfinished Wait calls are not reconstructed and waiting never retries Request Delivery.
_Avoid_: Answer cache, Request retry, durable Wait

**Agent Wait Preemption**:
The non-error completion of one Agent Wait when an eligible inbound Agent Request requires the parked exact Run. It reserves and commits that Request Delivery before the Agent's next model generation, restores Workflow execution capacity, and leaves every selected Answer available for a later Wait. Any inbound Agent Request may qualify independently of the Wait's selected responders; ordinary Deferred Messages remain queued.
