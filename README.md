...
- **Messaging and Requests:** `agent_message` supports immutable Deferred and Steer Messages, a foreground Request with causally suspended obligations, explicitly targeted Answers, retrieval, and cancellation. `agent_wait` joins outstanding outbound Request Answers owned by the foreground without consuming Workflow execution capacity when one next decision needs the complete set.
