...
- **Interactive Agent views:** `/agents` presents attention plus live and dormant Agents, starts one no-prompt successor when a Dormant Agent is selected, then attaches the selected Agent's complete Pi transcript, editor, footer, commands, and extension UI without rebinding the Owner runtime. A native above-editor dock keeps selected identity, scoped direct-child activity, and Owner-only attention visible after the selector closes.
