...
- **Messaging and Requests:** `agent_message` supports immutable Deferred and Steer Messages, Request attention ordering with Agent-chosen Answer order, explicitly targeted Answers, retrieval, and cancellation. `agent_wait` joins all outstanding outbound Request Answers or a selected list of Request IDs/unique suffixes, without consuming child execution capacity while parked.
