...
Each catalogue entry exposes its name, `use-when` guidance, configured frontmatter values, and only model candidates present in Pi's current model registry. A Template whose configured candidates are all unavailable is omitted. This lets the Agent select a Template or deliberately override values through `agent_spawn.config` for a context-isolated Spawn. A Conversation Fork uses neither. Invalid and ambiguous Templates, Template source paths, discovery diagnostics, and Markdown Project Context bodies are not exposed. The Snapshot is guidance only: every actual Spawn re-resolves the selected Template from current resources before preparing the child Runtime.
