...
Conversation inheritance is independent of Runtime configuration: a Conversation Fork may select a `template`, provide `config`, or both. Runtime preparation starts from parent inheritance, applies captured Template defaults, then explicit configuration overrides. Omit both `template` and `config` to preserve the parent setup and maximize cache reuse potential: the first request inherits the live parent's model, thinking, cwd, resources, and active tool surface while preserving the completed message prefix. Provider cache hits are not guaranteed. Configured forks use their resolved tool selection rather than the parent's active tool surface.
...
For either conversation mode, `config` may override the model/thinking pair, working directory, tool allowlist, skills, explicit system prompt, system-prompt mode, and native loading of trusted project instruction files such as `AGENTS.md` and `CLAUDE.md`. This context-file setting does not inherit the parent conversation. The canonical `config` remains in the parent Spawn call and overlays the captured Template rules whenever a fresh Runtime is prepared. A `config.model` object requires both `id` and `thinking`; either may be `inherit` to use that value from the current parent Runtime. `id: inherit` with `thinking: inherit` is valid. Providing `config.model` bypasses captured model candidates entirely. `allowedTools` and `skills` replace the captured or inherited selections, including with an empty array. Child extension selection is `inherit` or `none`; arbitrary per-child extension paths are not accepted.
...
The copied prefix, child Identity, and handoff are assembled in a Pi in-memory session, written once to a staging file outside discovery, validated from disk, and published together by atomic rename before the child process starts.
...
The child starts a fresh Pi CLI/TUI process and admits its fixed-Deferred Creation Request into the same serialized incoming-Request lane used by ordinary [Agent messaging](agent-messaging.md). Creation Request Delivery establishes the child's foreground obligation. Descendant Requests may suspend it at cooperative waiting boundaries; unrelated and sibling Requests remain queued. A successful spawn receipt reports volatile admission; it does not claim that Delivery committed, the model processed the Request, or an Answer exists.
