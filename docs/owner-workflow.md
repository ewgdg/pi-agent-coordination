...
- `workflow_resume` explicitly renews continuation intent for a fixed verified snapshot of the current Workflow. It restores original undelivered scheduling and reactivates dormant responders with delivered, unanswered Requests through runtime-generated continuation input, without redelivery or replacement Requests. Running Agents receive no duplicate continuation. The receipt reports admission, skips, and blocked/indeterminate work—not Delivery or completion. See [Workflow continuation](cold-host-recovery.md#explicit-workflow-continuation).
- `agent_wait` takes a fixed snapshot of every outstanding outbound Agent Request. It ensures original scheduling for captured unanswered, undelivered Requests through normal admission, including Dormant recipients; delivered Requests are awaited without replay and committed Answers remain retrievable from Dormant responders. Ongoing reconciliation respects Holds and exact recipient Run fences. It parks the exact Owner Run until the complete Answer set is ready, primary interactive human input redirects the Owner, or an eligible inbound Agent Request preempts the Wait for attention. Explicit follow-up input remains queued. Owner execution never consumes child execution capacity. A preempted Wait consumes no Answer; a later call takes a fresh outstanding snapshot.
