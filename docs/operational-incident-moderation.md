...
A Run Failure exists after one exact non-Moderator Run, including a Workflow Owner Run, ends unexpectedly after any available safe same-generation continuation and the Agent still has an unresolved Answer Obligation. The condition clears when every qualifying obligation ends or a successor Run successfully starts; a successor that later settles without progress is evaluated independently as an Obligation Stall.
...
Clean Run release, deliberate termination, orderly shutdown, optional work, ordinary model duration, Human waiting, and intentional Holds do not create Run Failure or Dependency Deadlock handling. Operation Review never times model generation or internal coordination machinery.
