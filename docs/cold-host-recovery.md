...
These are admission-time facts, not promises about later execution. Requests already verified resolved or not created are omitted. Unavailable evidence remains explicit; the Workflow-wide evidence report also covers failures that prevent identifying individual Requests. Continuations do not repeat incoming obligation identities or contents: restored Request relationships remain authoritative.
...
The call returns when recovery is admitted, not when the Workflow finishes. Its receipt identifies the `workflowId`, provides the Owner-scoped `outstandingRequests`, and retains the Workflow-wide operational report for inspection and rendering: `deliveries` (Message identity, target, kind, disposition, and optional reason), `activations` (Agent identity, Request identities, disposition, and optional reason), and Agent-level `indeterminate` evidence failures. Delivery dispositions are `scheduled`, `skipped`, `blocked`, or `indeterminate`; activation dispositions are `admitted`, `skipped`, `blocked`, or `indeterminate`. Admission is neither Delivery nor completion proof. Incomplete, unavailable, or contradictory evidence is reported rather than treated as proof that work is absent.
